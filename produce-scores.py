from utils import all_problems, all_problems_with_ratings, chosen_problems, jinja_env, QUALITY_CUTOFF

__version__ = "2024-02"

with open("final-report/table.tex", "w") as f:
    problems = all_problems_with_ratings()
    filtered_problems = [p for p in problems if p["quality_avg"] >= QUALITY_CUTOFF]
    if problems:
        env = jinja_env()
