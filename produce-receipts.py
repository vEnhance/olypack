...
with open('final-report/final-NO-SEND-report.tex') as f:
    ...
    for line in f:
        if line.startswith(r'\chapter{Comment'):
            ...
            slug = m.group(1)
        elif line.strip() == r'\end{description}' or line.startswith(
                r'\chapter'):
            feedback[slug] = current_comments
...
with open('output/summary.csv') as f:
    ...
    for row in reader:
        code = row[0]
        authors_string = row[2]
        authors_string = authors_string.replace(", and ", ", ")
        authors_string = authors_string.replace(" and ", ", ")
        authors = authors_string.split(", ")
        any_outside_authors = any(
            author not in report_audience for author in authors)
        s = '*' if any_outside_authors else ''
        ...
        if code in chosen:
            ...
            print(
                "If you notice anything you think should change, please let me know."
            )
            print(
            ...
            print(
                "meaning that you can now propose it to any other competition,")
            print("including next year if you so choose.")
            print(
                "If possible, please avoid discussing this problem decision with others"
            )
            print(
                "until after the exam, to avoid potentially leaking indirect information"
            )
